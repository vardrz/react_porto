...
function ProjectSection(){
    ...
    return (
        <div className="w-full pt-24 px-6 md:px-10 max-w-6xl mx-auto text-light">
            <div className="hero-text text-5xl sm:text-6xl tracking-tight leading-none mb-12">Featured Projects</div>

            {
                projects != null
                    ? projects.map((item, index) => (
                        <div key={index} className="w-full flex flex-col-reverse md:flex-row gap-8 py-12 border-b border-white/5 last:border-0">
                            <div className="flex flex-col md:w-3/4 justify-center">
                                <Link to={'/projects/' + item.slug} state={item} className="hero-text text-2xl sm:text-3xl text-primary hover:text-primary/80 transition-colors duration-200">{item.title}</Link>

                                <div className="flex mt-3">
                                    <div className="text-light/60 w-fit mr-10 flex flex-row items-center">
                                        <img src={"tech-icons/" + item.createBy + ".svg"} className="w-5 h-5" />
                                        <span className="text-sm ml-1">{item.createBy}</span>
                                    </div>
                                    {
                                        item.link != '-'
                                            ? (
                                                <a href={item.link} target="_blank" className="text-light/60 w-fit flex flex-row items-center hover:text-secondary transition-colors duration-200">
                                                    ...
                                                    <span className="text-sm ml-1 underline">Link</span>
                                                </a>
                                            )
                                            : ''
                                    }
                                ...
                                <div className="text-[15px] leading-relaxed text-light/75 mt-3 whitespace-pre-wrap line-clamp-3">{item.desc}</div>
